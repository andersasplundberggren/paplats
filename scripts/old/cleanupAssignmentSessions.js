...
async function main() {
  ...
  if (!assignmentId) {
    console.error('Du måste ange --assignment-id=<id>');
    process.exit(1);
  }
  ...
  if (!result) {
    console.error('Uppdraget hittades inte.');
    process.exit(1);
  }

  console.log('========================================');
  console.log('Manuell gallring av användarsessioner');
  console.log('========================================');
  console.log('Uppdrag ID: ' + result.assignment.id);
  console.log('Uppdrag: ' + result.assignment.title);
  console.log('Gallringsläge på uppdraget: ' + result.assignment.sessionRetentionMode);
...
main().catch(function (err) {
  console.error('[cleanupAssignmentSessions] Fel:', err);
  process.exit(1);
});