...
async function init() {
  ...
  // Hoppa över lagrad session i preview-läge
  if (!isPreview) {
    hydrateFromStorage();
  }
  queueMapRender();
...
function bindEvents() {
  ...
  if (els.joinGroupInput) {
    els.joinGroupInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') handleJoinGroup(); });
  }
  if (els.leaderboardFabBtn) {
    els.leaderboardFabBtn.addEventListener('click', openLeaderboard);
  }
  ...
  if (els.leaderboardBackdrop) {
    els.leaderboardBackdrop.addEventListener('click', closeLeaderboard);
  }
...
function hydrateFromQueryString() {
  ...
  // Preview-läge — admin förhandsgranskar utan GPS
  if (preview) {
    state.previewToken = String(preview).trim();
    state.isPreviewMode = true;
    loadPreviewAssignment(state.previewToken);
    return true; // signalerar preview-läge till init()
  }
  ...
  // Återuppta session direkt via token i URL
  if (session) {
...
async function loadPreviewAssignment(token) {
  try {
    ...
    state.assignment = normalizeAssignment({
      ...assignment,
      access_code: assignment.access_code || 'PREVIEW'
    });
...
function renderIntro(assignment) {
  ...
  // Visa gruppsektion och byt knapptext bara för tipspromenader
  const isTrail = assignment.type === 'trail';
  if (els.groupSection) {
    els.groupSection.classList.toggle('hidden', !isTrail);
  }
  if (els.startAssignmentBtn) {
    els.startAssignmentBtn.textContent = isTrail ? 'Spela enskilt' : 'Starta uppdrag';
  }

  // Återställ gruppstatus vid ny inläsning
  resetGroupSection();
}
...
async function handleCreateGroup() {
  ...
  try {
    // Skapa en temporär session enbart för att skapa gruppen
    // Den riktiga sessionen skapas när spelaren trycker "Spela enskilt"
    ...
    if (!sessionToken) {
      ...
      const accessCode = String(
        state.assignmentCode || state.assignment?.access_code || els.codeInput?.value || ''
      ).trim().toUpperCase();
      ...
      sessionToken =
        sessionResp?.data?.session_token || sessionResp?.session_token ||
        sessionResp?.data?.token || sessionResp?.token;
...
async function handleJoinGroup() {
  ...
  if (!code) {
    showGroupStatus('Ange en gruppkod.', 'error');
    els.joinGroupInput?.focus();
    return;
  }
...
function updateLeaderboardBtnVisibility() {
  ...
  const show = isInGroup() && !!state.sessionToken;
  els.leaderboardFabBtn.classList.toggle('hidden', !show);
}
...
function closeLeaderboard() {
  ...
  if (els.leaderboardBackdrop) {
    els.leaderboardBackdrop.classList.remove('active');
  }
}
...
async function loadLeaderboard() {
  ...
  try {
    ...
    let html = `<div class="leaderboard-group-info">
      <span class="leaderboard-group-name">${group_name || ''}</span>
      <span class="leaderboard-group-code">${invite_code || ''}</span>
    </div><div class="leaderboard-list">`;
    ...
    if (!leaderboard || leaderboard.length === 0) {
      ...
      leaderboard.forEach(row => {
        ...
        html += `<div class="leaderboard-row ${meClass}">
          <div class="leaderboard-rank ${rankClass}">${row.rank}</div>
          <div style="flex:1">
            <div class="leaderboard-name">${row.display_name}${row.is_me ? ' (du)' : ''}</div>
...
async function loadProgress(preserveOnError = false) {
  ...
  // Preview — behåll lokal progress, kontakta inte servern
...
async function handleRegisterVisit(placeId, button, options = {}) {
  ...
  try {
    if (state.isPreviewMode) {
      // Preview — spara lokalt utan server
...
function getPlaceAvailability(place) {
  ...
  // Preview-läge — alla platser är tillgängliga utan GPS
  if (state.isPreviewMode) {
    ...
    return {
      visited,
      unlockMode: getUnlockMode(place),
      requiresLocation: false,
      distance: 0,
      unlockDistance: 0,
      pendingPosition: false,
      sequenceBlocked,
      previousPlace: sequenceBlocked ? previousPlace : null,
...
async function submitQuestionAnswer(place, block, optionKey) {
  ...
  try {
    ...
    if (state.isPreviewMode) {
      const isCorrect = String(block.correct_option || '').toUpperCase() === String(optionKey).toUpperCase();
      result = {
        is_correct: isCorrect ? 1 : 0,
        points_earned: isCorrect ? (block.points || 0) : 0
      };
    } else {
      const response = await fetchJson(`${apiBaseUrl}/sessions/${encodeURIComponent(state.sessionToken)}/progress`, {
        method: 'POST',
