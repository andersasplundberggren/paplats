...
/* ============================================================
   PÅ PLATS — adapter för v2 HTML
   Körs efter originalets kod och mappar om:
   - Vyvisning (showView) till v2:s CSS-klasser
   - Element-ID:n till v2:s struktur
   - Bottom nav-interaktion
   - Sheet-hantering
   ============================================================ */

(function paplatsAdapter() {
  'use strict';
  ...
  function init() {
    ...
    // ── 1. Patch showView ──────────────────────────────────
    // Originalet använder data-view på body + .active på vyer
    // V2:s HTML har samma mönster men med andra ID:n
    ...
    window.showView = function(name) {
      ...
      if (listSheet)   listSheet.classList.remove('active');
      ...
      const targetId = viewMap[name] || 'viewCode';
      const el = document.getElementById(targetId);
      if (el) el.classList.add('active');

      // Platsdetalj = öppna detailSheet
      if (name === 'placeDetail') {
      ...
      // Trigga kartrendering
      if ((name === 'map' || name === 'placeDetail') && typeof queueMapRender === 'function') {
        queueMapRender();
      }
    };

    // ── 2. Patch setActivePanel ────────────────────────────
    const _origSetActivePanel = window.setActivePanel;
    window.setActivePanel = function(panel) {
      if (typeof _origSetActivePanel === 'function') {
        _origSetActivePanel(panel);
      }
      ...
      // Uppdatera sheets
      if (listSheet)   listSheet.classList.toggle('active', panel === 'list');
      if (detailSheet && panel !== 'detail') detailSheet.classList.remove('active');

      // Uppdatera bottom nav
      document.querySelectorAll('.bottom-nav-item').forEach(btn => btn.classList.remove('active'));
      const panelBtnMap = { map: 'navMap', list: 'navList', detail: 'navList' };
      const activeBtn = document.getElementById(panelBtnMap[panel] || 'navMap');
      if (activeBtn) activeBtn.classList.add('active');
    };

    // ── 3. Synka element-ID:n ──────────────────────────────
    // Originalet läser el.mapCanvas, el.listPanel etc.
    // V2:s HTML har delvis andra ID:n — vi patchar els-objektet
    // efter att originalet har initialiserat det.
    setTimeout(function syncEls() {
      if (typeof els === 'undefined') return;

      // Kod-vy
      if (!els.codeInput)         els.codeInput         = document.getElementById('codeInput');
      if (!els.loadAssignmentBtn) els.loadAssignmentBtn = document.getElementById('loadAssignmentBtn');
      if (!els.nicknameInput)     els.nicknameInput     = document.getElementById('nicknameInput');
      if (!els.startAssignmentBtn)els.startAssignmentBtn= document.getElementById('startAssignmentBtn');
      if (!els.resumeCard)        els.resumeCard        = document.getElementById('resumeCard');
      if (!els.resumeBtn)         els.resumeBtn         = document.getElementById('resumeBtn');
      if (!els.installBtn)        els.installBtn        = document.getElementById('installBtn');

      // Vyer
      els.codeView        = document.getElementById('viewCode');
      els.introView       = document.getElementById('viewIntro');
      els.mapView         = document.getElementById('viewMap');
      els.placeDetailView = document.getElementById('detailSheet');
      els.completionView  = document.getElementById('viewCompletion');

      // Intro
      els.assignmentType      = document.getElementById('introAssignmentType');
      els.assignmentTitle     = document.getElementById('introTitle');
      els.assignmentIntroTitle= document.getElementById('introSubtitle');
      els.assignmentIntro     = document.getElementById('introDescription');
      els.backToCodeBtn       = document.getElementById('introBackBtn');

      // Karta
      els.activeAssignmentTitle = document.getElementById('mapAssignmentTitle');
      els.mapCanvas             = document.getElementById('mapCanvas');
      els.mapStatusText         = document.getElementById('mapGpsText');
      els.locateMeBtn           = document.getElementById('mapLocateBtn');
      els.zoomInBtn             = document.getElementById('mapZoomInBtn');
      els.zoomOutBtn            = document.getElementById('mapZoomOutBtn');
      els.mapIntroCard          = document.getElementById('mapIntroCard');
      els.mapIntroCardTitle     = document.getElementById('mapIntroCardTitle');
      els.mapIntroCardBody      = document.getElementById('mapIntroCardBody');
      els.closeMapIntroBtn      = document.getElementById('mapIntroCloseBtn');
      els.fitAllBtn             = document.getElementById('mapLocateBtn');

      // Lista
      els.listPanel         = document.getElementById('listSheet');
      els.listSheetTitle    = document.getElementById('listSheetTitle');
      els.listSheetSubtitle = document.getElementById('listSheetSubtitle');
      els.placesList        = document.getElementById('placesList');
      els.toggleListBtn     = document.getElementById('navList');
      els.closeListBtn      = document.getElementById('listSheetBackBtn');

      // Detalj
      els.detailPlaceTitle  = document.getElementById('detailSheetTitle');
      els.detailContentList = document.getElementById('detailSheetBody');
      els.backToMapBtn      = document.getElementById('detailSheetBackBtn');
      els.backToListBtn     = document.getElementById('detailSheetBackBtn');

      // Anteckningar (v2 har viewNotes som separat vy)
      els.notesFabBtn  = document.getElementById('navNotes');
      els.notesOverlay = document.getElementById('viewNotes');
      els.notesTextarea= document.getElementById('notesTextarea');
      els.closeNotesBtn= document.getElementById('notesBackBtn');

      // Topplista
      els.leaderboardFabBtn    = document.getElementById('navLeaderboard');
      els.leaderboardOverlay   = document.getElementById('leaderboardOverlay');
      els.leaderboardTitle     = document.getElementById('leaderboardTitle');
      els.leaderboardBody      = document.getElementById('leaderboardBody');
      els.closeLeaderboardBtn  = document.getElementById('leaderboardCloseBtn');
      els.refreshLeaderboardBtn= document.getElementById('leaderboardRefreshBtn');

      // Avslutning
      els.completionLead       = document.getElementById('completionSummary');
      els.leaveAssignmentBtn   = document.getElementById('navLeave');

      // Grupp
      els.groupSection    = document.getElementById('groupSection');
      els.createGroupBtn  = document.getElementById('createGroupBtn');
      els.joinGroupInput  = document.getElementById('joinGroupInput');
      els.joinGroupBtn    = document.getElementById('joinGroupBtn');
      els.groupCreated    = document.getElementById('groupCreated');
      els.groupInviteCode = document.getElementById('groupInviteCode');
      els.groupStatus     = document.getElementById('groupStatus');

      // Stub-element som originalet kräver men v2 inte har
      function stub(id) {
      ...
      els.screenTitle          = stub('screenTitle');
      els.listBackdrop         = stub('listBackdrop');
      els.detailBackdrop       = stub('detailBackdrop');
      els.notesBackdrop        = stub('notesBackdrop');
      els.leaderboardBackdrop  = stub('leaderboardBackdrop');
      els.completionBackBtn    = document.getElementById('completionDoneBtn');
      els.completionCancelBtn  = stub('completionCancelBtn');
      els.completionBackdrop   = stub('completionBackdrop');
      els.completionConfirmBtn = document.getElementById('completionDoneBtn');
      els.assignmentTypeBadge  = stub('assignmentTypeBadge');
      els.assignmentPrimaryAction = stub('assignmentPrimaryAction');
      els.assignmentMysteryHint= stub('assignmentMysteryHint');
      els.assignmentPlaceCount = stub('assignmentPlaceCount');
      els.assignmentVisitedCount = stub('assignmentVisitedCount');
      els.detailHeroMedia      = stub('detailHeroMedia');
      els.detailPlaceDescription = stub('detailPlaceDescription');
      els.detailPlaceOrder     = stub('detailPlaceOrder');
      els.detailCtaRow         = stub('detailCtaRow');
      els.detailRegisterVisitBtn = stub('detailRegisterVisitBtn');
      els.notesStatus          = stub('notesStatus');
      els.notesTabBtn          = stub('notesTabBtn');
      els.evidenceTabBtn       = stub('evidenceTabBtn');
      els.evidenceTabContent   = stub('evidenceTabContent');
      els.notesTabContent      = stub('notesTabContent');
      els.infoFabBtn           = stub('infoFabBtn');
      els.evidenceFabBtn       = stub('evidenceFabBtn');
      els.mapDistanceHint      = stub('mapDistanceHint');
      els.mapCountdown         = stub('mapCountdown');
      els.openStartGuideBtn    = stub('openStartGuideBtn');
      els.closeStartGuideBtn   = stub('closeStartGuideBtn');
      els.startGuideCard       = stub('startGuideCard');
      els.resetSessionBtn      = document.getElementById('resumeClearBtn');
      els.resumeAssignmentTitle = document.getElementById('resumeTitle');
      els.resumeAssignmentCode = stub('resumeAssignmentCode');
      els.resumeDismissBtn     = document.getElementById('resumeClearBtn');
      els.completionPlacesStat = stub('completionPlacesStat');
      els.completionAnswersStat= stub('completionAnswersStat');
      els.completionCorrectStat= stub('completionCorrectStat');
      els.completionScoreStat  = stub('completionScoreStat');
      els.completionModeNotice = stub('completionModeNotice');
      els.completionRecentList = stub('completionRecentList');
      els.autoCompleteView     = stub('autoCompleteView');
      els.autoCompleteBackdrop = stub('autoCompleteBackdrop');
      els.autoCompleteTitle    = stub('autoCompleteTitle');
      els.autoCompleteLead     = stub('autoCompleteLead');
      els.autoCompleteSummaryWrap = stub('autoCompleteSummaryWrap');
      els.autoCompletePlacesStat = stub('autoCompletePlacesStat');
      els.autoCompleteAnswersStat = stub('autoCompleteAnswersStat');
      els.autoCompleteCorrectStat = stub('autoCompleteCorrectStat');
      els.autoCompleteScoreStat = stub('autoCompleteScoreStat');
      els.autoCompleteModeNotice = stub('autoCompleteModeNotice');
      els.autoCompleteRecentList = stub('autoCompleteRecentList');
      els.autoCompleteSummaryToggleBtn = stub('autoCompleteSummaryToggleBtn');
      els.autoCompleteFinishBtn = stub('autoCompleteFinishBtn');
      els.autoCompleteContinueBtn = stub('autoCompleteContinueBtn');
      els.placeTemplate        = stub('placeTemplate');
      els.digitalView          = stub('digitalView');
      els.digitalCountdown     = stub('digitalCountdown');
      els.fitAllBtn            = document.getElementById('mapLocateBtn');

    }, 100);

    // ── 4. Bottom nav event-lyssnare ──────────────────────
    setTimeout(function bindBottomNav() {

      // Karta
      const navMap = document.getElementById('navMap');
      if (navMap) navMap.addEventListener('click', function() {
        if (typeof setActivePanel === 'function') setActivePanel('map');
      ...
      if (navList) navList.addEventListener('click', function() {
        if (typeof setActivePanel === 'function') setActivePanel('list');
        document.querySelectorAll('.bottom-nav-item').forEach(b => b.classList.remove('active'));
        navList.classList.add('active');
      });

      // Anteckningar — öppnar viewNotes som separat vy
      const navNotes = document.getElementById('navNotes');
      if (navNotes) navNotes.addEventListener('click', function() {
        if (typeof openNotesPanel === 'function') {
          openNotesPanel('notes');
        } else {
          // Fallback: visa viewNotes
          document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
          const notesView = document.getElementById('viewNotes');
          if (notesView) notesView.classList.add('active');
      ...
      if (navLeaderboard) navLeaderboard.addEventListener('click', function() {
        if (typeof openLeaderboard === 'function') openLeaderboard();
      });

      // Lämna
      const navLeave = document.getElementById('navLeave');
      if (navLeave) navLeave.addEventListener('click', function() {
        if (typeof handleLeaveAssignment === 'function') handleLeaveAssignment();
      });

      // Tillbaka från lista-sheet
      const listBack = document.getElementById('listSheetBackBtn');
      ...
      // Tillbaka från detalj-sheet
      const detailBack = document.getElementById('detailSheetBackBtn');
      ...
      // Tillbaka från anteckningsvyn
      const notesBack = document.getElementById('notesBackBtn');
      if (notesBack) notesBack.addEventListener('click', function() {
        if (typeof closeNotesPanel === 'function') {
          closeNotesPanel();
        } else {
          document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
      ...
      if (leaderboardClose) leaderboardClose.addEventListener('click', function() {
        if (typeof closeLeaderboard === 'function') closeLeaderboard();
      });

      // Uppdatera topplista
      const leaderboardRefresh = document.getElementById('leaderboardRefreshBtn');
      if (leaderboardRefresh) leaderboardRefresh.addEventListener('click', function() {
        if (typeof loadLeaderboard === 'function') loadLeaderboard();
      });

      // Avslutning klar
      const completionDone = document.getElementById('completionDoneBtn');
      if (completionDone) completionDone.addEventListener('click', function() {
        if (typeof resetLocalSession === 'function') resetLocalSession();
        if (typeof showView === 'function') showView('code');
      });

      // Intro tillbaka
      const introBack = document.getElementById('introBackBtn');
      if (introBack) introBack.addEventListener('click', function() {
        if (typeof showView === 'function') showView('code');
      });

    }, 200);

    // ── 5. Patch openNotesPanel för v2 ────────────────────
    setTimeout(function patchNotes() {
      const _origOpen  = window.openNotesPanel;
      const _origClose = window.closeNotesPanel;

      window.openNotesPanel = function(tab) {
        document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
        const notesView = document.getElementById('viewNotes');
        if (notesView) notesView.classList.add('active');
        document.body.dataset.view = 'notes';
        ...
        // Tab-switching
        if (tab) {
          const tabMap = { notes: 'free', evidence: 'evidence', suspects: 'suspects' };
          const t = tabMap[tab] || 'free';
          document.querySelectorAll('#notesTabs .tab').forEach(el => {
            el.classList.toggle('active', el.dataset.tab === t);
          });
          document.querySelectorAll('.tab-panel').forEach(el => el.classList.remove('active'));
          const panels = { free: 'tabFree', evidence: 'tabEvidence', suspects: 'tabSuspects' };
    ...
    // ── 6. Tab-switching i anteckningsvyn ─────────────────
    setTimeout(function bindTabs() {
      document.querySelectorAll('#notesTabs .tab').forEach(function(tab) {
        tab.addEventListener('click', function() {
          document.querySelectorAll('#notesTabs .tab').forEach(t => t.classList.remove('active'));
          tab.classList.add('active');
          const target = tab.dataset.tab;
          document.querySelectorAll('.tab-panel').forEach(p => p.classList.remove('active'));
          const panelMap = { suspects: 'tabSuspects', evidence: 'tabEvidence', free: 'tabFree' };
    ...
    // ── 7. Spara anteckningar-knapp ───────────────────────
    setTimeout(function bindSaveNotes() {
      const saveBtn = document.getElementById('saveNotesBtn');
      if (saveBtn) saveBtn.addEventListener('click', function() {
        if (typeof saveNotesNow === 'function') saveNotesNow();
      });
    }, 250);

    // ── 8. Lightbox ───────────────────────────────────────
    setTimeout(function bindLightbox() {
      ...
      if (lbClose) lbClose.addEventListener('click', function() {
        ...
        else if (lb) lb.setAttribute('hidden', '');
      });
    ...
    // ── 9. Dela gruppkod ──────────────────────────────────
    setTimeout(function bindShareGroup() {
      const shareBtn  = document.getElementById('shareGroupCodeBtn');
      const modal     = document.getElementById('shareGroupModal');
      const closeBtn  = document.getElementById('shareGroupCloseBtn');
      const codeEl    = document.getElementById('shareGroupCode');
      const copyBtn   = document.getElementById('copyGroupCodeBtn');
      const nativeBtn = document.getElementById('shareGroupNativeBtn');
      ...
      if (closeBtn && modal) closeBtn.addEventListener('click', function() { if (modal.close) modal.close(); });
      if (copyBtn) {
      ...
      if (nativeBtn) {
        nativeBtn.addEventListener('click', async function() {
          const code = codeEl ? codeEl.textContent : '';
          if (navigator.share) {
            try { await navigator.share({ text: 'Gå med i min grupp i På plats! Kod: ' + code }); } catch {}
          } else {
            try { await navigator.clipboard.writeText(code); } catch {}
          }
        });
    ...
    // ── 10. statusCard som toast i v2 ─────────────────────
    setTimeout(function patchStatus() {
      const sc = document.getElementById('statusCard');
      const toast = document.getElementById('toast');
      if (!sc || !toast) return;

      // Observera statusCard och speila till toast
      const obs = new MutationObserver(function() {
        const text = (sc.textContent || '').trim();
        const isError = sc.style.background && sc.style.background.includes('220, 38');
        if (text) {
          toast.textContent = text;
          toast.className = 'toast visible' + (isError ? ' is-error' : '');
      ...
      obs.observe(sc, { childList: true, characterData: true, subtree: true, attributes: true });
    }, 150);

    // ── 11. SPOTON_CONFIG → PAPLATS_CONFIG ────────────────
    // Säkerställ att config är satt korrekt
    if (!window.SPOTON_CONFIG) {
      var src = window.PAPLATS_CONFIG || {};
      window.SPOTON_CONFIG = {
        apiBaseUrl:   src.apiBaseUrl   || '/api/v1',
        appBasePath:  '/app/pwa',
        mediaBaseUrl: src.mediaBaseUrl || '',
        map: {
          tileUrlTemplate:     (src.map && src.map.tileUrlTemplate)     || 'https://tile.openstreetmap.org/{z}/{x}/{y}.png',
          defaultZoom:         (src.map && src.map.defaultZoom)         || 15,
          minZoom:             (src.map && src.map.minZoom)             || 3,
          maxZoom:             (src.map && src.map.maxZoom)             || 18,
          nearDistanceMeters:  (src.map && src.map.nearDistanceMeters)  || 120,
          closeDistanceMeters: (src.map && src.map.closeDistanceMeters) || 40
        }
      };
    }
