...
const els = {
  ...
  locationGuideBtn: document.getElementById('locationGuideBtn'),
  closeLocationGuideBtn: document.getElementById('closeLocationGuideBtn'),
  locationGuideCard: document.getElementById('locationGuideCard'),
  ...
  evidenceTabBtn: document.getElementById('evidenceTabBtn'),
  evidenceList:   document.getElementById('evidenceList'),
  suspectTabBtn:  document.getElementById('suspectTabBtn'),
  suspectList:    document.getElementById('suspectList'),
  infoFabBtn: document.getElementById('infoFabBtn'),
  evidenceFabBtn: document.getElementById('evidenceFabBtn'),
  ...
  bottomNavLeaderboardBtn: document.getElementById('bottomNavLeaderboardBtn')
};
...
function bindEvents() {
  // ═══════════════════════════════════════════════════════════════
  // VIKTIGT — LÄS INNAN DU ÄNDRAR DENNA FUNKTION
  //
  // Flera element kan saknas i DOM:en beroende på vilken version av
  // index.html som körs. Använd ALLTID null-skydd (if-sats) innan
  // addEventListener anropas. En saknad referens kraschar hela appen.
  //
  // Element som KAN saknas (kräver null-skydd):
  //   els.toggleListBtn      — borttagen i nya index.html (bottom nav används)
  //   els.closeListBtn       — kan saknas
  //   els.listBackdrop       — kan saknas
  //   els.notesFabBtn        — dold/saknas beroende på spelläge
  //   els.evidenceFabBtn     — dold/saknas beroende på spelläge
  //   els.leaderboardFabBtn  — dold/saknas beroende på spelläge
  //   els.bottomNav          — saknas i gamla index.html
  //
  // Element som ALLTID finns (säkra utan null-skydd):
  //   els.loadAssignmentBtn, els.startAssignmentBtn, els.backToCodeBtn
  //   els.leaveAssignmentBtn, els.completionBackBtn, els.locateMeBtn
  //
  // Checklist att köra vid varje session som rör denna fil:
  //   grep -q "if (els.toggleListBtn)" app.js && echo "OK" || echo "SAKNAS"
  //   grep -q "evidenceList:.*getElementById" app.js && echo "OK" || echo "SAKNAS"
  //   grep -q "bottomNav:.*getElementById" app.js && echo "OK" || echo "SAKNAS"
  //   grep -q "^function updateBottomNav" app.js && echo "OK" || echo "SAKNAS"
  //   grep -q "openLightbox.*witness_image_url" app.js && echo "OK" || echo "SAKNAS"
  //   grep -q "openLightbox.*media_url" app.js && echo "OK" || echo "SAKNAS"
  //   grep -q "if (els.closeListBtn)" app.js && echo "OK" || echo "SAKNAS"
  // ═══════════════════════════════════════════════════════════════
  ...
  if (els.locationGuideBtn) {
    els.locationGuideBtn.addEventListener('click', openLocationGuide);
  }
  if (els.closeLocationGuideBtn) {
    els.closeLocationGuideBtn.addEventListener('click', closeLocationGuide);
  }
  ...
  if (els.notesTabBtn) els.notesTabBtn.addEventListener('click', () => switchNotesTab('notes'));
  if (els.evidenceTabBtn) els.evidenceTabBtn.addEventListener('click', () => switchNotesTab('evidence'));
  if (els.suspectTabBtn) els.suspectTabBtn.addEventListener('click', () => switchNotesTab('suspects'));
  ...
  if (els.autoCompleteContinueBtn) {
    els.autoCompleteContinueBtn.addEventListener('click', () => {
      state.readMode = true;
  ...
  if (els.closeListBtn) els.closeListBtn.addEventListener('click', () => setActivePanel('map'));
  if (els.listBackdrop) els.listBackdrop.addEventListener('click', () => setActivePanel('map'));
  ...
  if (els.bottomNav) {
    els.bottomNav.addEventListener('click', (e) => {
      ...
      if      (nav === 'map')         setActivePanel('map');
      else if (nav === 'list')        setActivePanel(state.activePanel === 'list' ? 'map' : 'list');
      else if (nav === 'notes')       toggleNotesPanel();
      else if (nav === 'evidence')    openNotesPanel('evidence');
      else if (nav === 'leaderboard') openLeaderboard();
      else if (nav === 'leave')       handleLeaveAssignment();
    });
  }
  ...
  // Stoppa klick/touch inne i platskortet från att bubbla upp till bakgrunden
  // — annars stängs detaljvyn direkt när användaren trycker inuti kortet.
  const _detailSheet = document.querySelector('#placeDetailView .detail-sheet');
  if (_detailSheet) {
    _detailSheet.addEventListener('click', (e) => e.stopPropagation());
    _detailSheet.addEventListener('touchstart', (e) => e.stopPropagation(), { passive: true });
  }
  els.detailRegisterVisitBtn.addEventListener('click', handleRegisterVisitFromDetail);
  if (els.infoFabBtn) {
    els.infoFabBtn.addEventListener('click', () => {
      if (els.mapIntroCard) els.mapIntroCard.classList.toggle('hidden');
    });
  }
  if (els.evidenceFabBtn) {
    els.evidenceFabBtn.addEventListener('click', () => openNotesPanel('evidence'));
  }
  if (els.notesFabBtn) {
...
function updateNotesVisibility() {
  ...
  if (els.evidenceFabBtn) {
    els.evidenceFabBtn.classList.toggle('hidden', !show);
  }
...
function updateBottomNav() {
  ...
  els.bottomNav.classList.toggle('is-visible', !!state.sessionToken);

  const mode = state.assignment ? getAssignmentMode() : null;
  document.body.dataset.mode = mode || '';

  els.bottomNav.querySelectorAll('[data-nav]').forEach(btn => {
    const nav = btn.dataset.nav;
    btn.classList.toggle('is-active',
      (nav === 'map'   && state.activePanel === 'map') ||
      (nav === 'list'  && state.activePanel === 'list') ||
      (nav === 'notes' && state.activePanel === 'notes')
    );
  });

  if (els.bottomNavLeaderboardBtn) {
    els.bottomNavLeaderboardBtn.style.display =
      isTrailAssignment() && isInGroup() ? '' : 'none';
  }
}
...
function openNotesPanel(tab = 'notes') {
  ...
  // Byt flik efter att panelen visats så DOM:en är redo
  requestAnimationFrame(() => switchNotesTab(tab));
}
...
function switchNotesTab(tab) {
  ...
  const suspectContent  = document.getElementById('suspectTabContent');

  if (els.notesTabBtn)   els.notesTabBtn.classList.toggle('notes-tab-active',   tab === 'notes');
  if (els.evidenceTabBtn) els.evidenceTabBtn.classList.toggle('notes-tab-active', tab === 'evidence');
  if (els.suspectTabBtn)  els.suspectTabBtn.classList.toggle('notes-tab-active',  tab === 'suspects');

  if (notesContent)    notesContent.style.display    = tab === 'notes'    ? '' : 'none';
  if (evidenceContent) evidenceContent.style.display = tab === 'evidence' ? '' : 'none';
  if (suspectContent)  suspectContent.style.display  = tab === 'suspects' ? '' : 'none';
  ...
  if (tab === 'suspects') renderSuspectList();
}
...
function renderEvidenceList() {
  ...
  for (const { block } of collected) {
    ...
    if (block.media_url) {
      ...
      img.addEventListener('click', () => openLightbox(block.media_url, img.alt));
...
function collectSuspects() {
  ...
  for (const place of getPlaces()) {
    if (!hasVisitedPlace(place.id)) continue;
    for (const block of getContentBlocks(place)) {
      if (normalizeBlockType(block.type) === 'suspect') {
        suspects.push({ block, place });
      }
    }
  }
  return suspects;
...
function renderSuspectList() {
  const container = els.suspectList;
  if (!container) return;
  ...
  if (suspects.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'evidence-empty';
    empty.textContent = 'Inga misstänkta har lagts till i det här uppdraget.';
    container.appendChild(empty);
    return;
  }
  ...
  for (const { block } of suspects) {
    ...
    // Header: foto + namn + roll
    const header = document.createElement('div');
    header.className = 'suspect-card-header';

    if (block.suspect_image_url) {
      const img = document.createElement('img');
      img.src = block.suspect_image_url;
      img.alt = block.suspect_name || 'Misstänkt';
      img.className = 'suspect-card-img';
      img.addEventListener('click', () => openLightbox(block.suspect_image_url, img.alt));
      header.appendChild(img);
    } else {
      const avatar = document.createElement('div');
      avatar.className = 'suspect-card-avatar';
      avatar.textContent = block.suspect_name ? block.suspect_name.charAt(0).toUpperCase() : '?';
      header.appendChild(avatar);
    }

    const nameWrap = document.createElement('div');
    nameWrap.className = 'suspect-card-name-wrap';
    if (block.suspect_name) {
      const name = document.createElement('div');
      name.className = 'suspect-card-name';
      name.textContent = block.suspect_name;
      nameWrap.appendChild(name);
    }
    if (block.suspect_role) {
      const role = document.createElement('div');
      role.className = 'suspect-card-role';
      role.textContent = block.suspect_role;
    ...
    badge.className = 'suspect-card-badge';
    if (block.is_culprit) {
      badge.classList.add('suspect-badge-suspect');
      badge.textContent = 'Misstänkt';
    } else {
      badge.classList.add('suspect-badge-uncertain');
      badge.textContent = 'Osäker';
    }
    ...
    // Motiv
    if (block.suspect_motive) {
      const motiveWrap = document.createElement('div');
      motiveWrap.className = 'suspect-card-section';
      const motiveLabel = document.createElement('div');
      motiveLabel.className = 'suspect-card-label';
      motiveLabel.textContent = 'Motiv';
      const motiveText = document.createElement('div');
      motiveText.className = 'suspect-card-text';
      motiveText.textContent = block.suspect_motive;
      motiveWrap.appendChild(motiveLabel);
      motiveWrap.appendChild(motiveText);
      card.appendChild(motiveWrap);
    }

    // Alibi
    if (block.suspect_alibi) {
      const alibiWrap = document.createElement('div');
      alibiWrap.className = 'suspect-card-section';
      const alibiLabel = document.createElement('div');
      alibiLabel.className = 'suspect-card-label';
      alibiLabel.textContent = 'Alibi';
      const alibiText = document.createElement('div');
      alibiText.className = 'suspect-card-text';
      alibiText.textContent = block.suspect_alibi;
      alibiWrap.appendChild(alibiLabel);
      alibiWrap.appendChild(alibiText);
      card.appendChild(alibiWrap);
    }
...
function openLeaderboard() {
  ...
  if (els.leaderboardBackdrop) els.leaderboardBackdrop.classList.add('active');
  loadLeaderboard();
  // Auto-refresh var 30:e sekund medan panelen är öppen
  if (state._leaderboardInterval) clearInterval(state._leaderboardInterval);
  state._leaderboardInterval = setInterval(loadLeaderboard, 30000);
}
...
function closeLeaderboard() {
  ...
  if (state._leaderboardInterval) {
    clearInterval(state._leaderboardInterval);
    state._leaderboardInterval = null;
  }
}
...
async function loadProgress(preserveOnError = false) {
  ...
  try {
    ...
    // Återställ gruppinformation om sessionen tillhör en grupp
    if (payload.event_id && !state.activeGroup) {
      // Hämta gruppinfo från leaderboard-API:et
      try {
        const url = `${apiBaseUrl}/events/${payload.event_id}/leaderboard?session_token=${encodeURIComponent(state.sessionToken)}`;
        const groupResp = await fetchJson(url);
        if (groupResp.ok) {
          state.activeGroup = {
            event_id:    payload.event_id,
            invite_code: groupResp.invite_code || '',
            group_name:  groupResp.group_name  || ''
          };
          updateBottomNav();
        }
      } catch (e) {
        // Gruppen kan ha gått ut — ignorera tyst
      }
    }
...
function getAssignmentMode() {
  if (isMysteryAssignment()) return 'mystery';
  if (isTrailAssignment())   return 'trail';
  return 'standard';
}

function renderAssignmentView() {
  document.body.dataset.mode = getAssignmentMode();
  renderPlaces();
...
function handleAssignmentPrimaryAction() {
  ...
  // Trail med alla platser klara — visa autoComplete-overlaynen som avslutningsflöde
  if (allVisited && isTrailAssignment()) {
    showAutoCompleteView();
    return;
  }

  // Om alla platser är besökta (andra lägen) — gå till avslutningsvyn
  if (allVisited) {
...
function updateAssignmentSummary() {
  ...
  if (els.assignmentPrimaryAction) {
    ...
    if (allVisited) {
      els.assignmentPrimaryAction.textContent = trailMode ? 'Avsluta tipspromenaden →' : 'Avsluta uppdraget →';
  ...
  if (els.assignmentMysteryHint) {
    ...
    if (allVisited && trailMode) {
      els.assignmentMysteryHint.textContent = 'Alla steg i tipspromenaden är klara.';
    } else if (action?.hint) {
...
async function handleLeaveAssignment() {
  ...
  // Mystery med alla platser klara — visa gissningsflödet
  if (allDone && isMysteryAssignment()) {
    showMysteryGuessModal();
    return;
  }
...
// ─── Mordgåta: gissningsflöde ─────────────────────────────────
function showMysteryGuessModal() {
  const suspects = [];
  for (const place of getPlaces()) {
    for (const block of getContentBlocks(place)) {
      if (normalizeBlockType(block.type) === 'suspect') {
  ...
  const existing = document.getElementById('mysteryGuessPanel');
  if (existing) existing.remove();

  // Bygg panel i samma stil som platsdetaljvyn
  const overlay = document.createElement('div');
  overlay.id = 'mysteryGuessPanel';
  ...
  handle.style.cssText = 'width:40px;height:4px;border-radius:2px;background:rgba(0,0,0,.15);margin:12px auto 0;flex-shrink:0';
  sheet.appendChild(handle);

  // Header
  const header = document.createElement('div');
  header.style.cssText = 'padding:16px 20px 0;flex-shrink:0';
  const title = document.createElement('h2');
  title.style.cssText = 'font-size:1.15rem;font-weight:700;color:#1a202c;margin:0 0 4px';
  title.textContent = '🔍 Vem är skyldig?';
  const lead = document.createElement('p');
  lead.style.cssText = 'font-size:.85rem;color:#64748b;margin:0 0 16px;line-height:1.5';
  lead.textContent = 'Du har besökt alla platser. Välj den person du tror är mördaren och bekräfta.';
  header.appendChild(title);
  ...
  // Scrollbar innehåll
  const body = document.createElement('div');
  body.style.cssText = 'overflow-y:auto;flex:1;padding:0 20px';

  let selectedId = null;

  if (suspects.length === 0) {
    const empty = document.createElement('p');
    empty.style.cssText = 'font-size:.88rem;color:#64748b;text-align:center;padding:1rem 0';
    empty.textContent = 'Inga misstänkta har lagts till i uppdraget.';
    body.appendChild(empty);
  } else {
    const list = document.createElement('div');
    list.style.cssText = 'display:grid;gap:10px;margin-bottom:16px';

    suspects.forEach(block => {
      const row = document.createElement('button');
      row.type = 'button';
      row.style.cssText = 'display:flex;align-items:center;gap:14px;padding:12px 16px;border:2px solid #e2e8f0;border-radius:16px;background:#f8fafc;cursor:pointer;text-align:left;width:100%;transition:border-color .15s,background .15s';
      ...
      if (block.suspect_image_url) {
        ...
        av.style.cssText = 'width:48px;height:48px;border-radius:50%;background:rgba(157,23,77,.1);color:#9d174d;display:flex;align-items:center;justify-content:center;font-size:1.2rem;font-weight:700;flex-shrink:0';
        av.textContent = block.suspect_name ? block.suspect_name.charAt(0).toUpperCase() : '?';
        row.appendChild(av);
      }
      ...
      const name = document.createElement('div');
      name.style.cssText = 'font-size:.95rem;font-weight:600;color:#1a202c';
      name.textContent = block.suspect_name || 'Okänd';
      info.appendChild(name);
      if (block.suspect_role) {
        const role = document.createElement('div');
        role.style.cssText = 'font-size:.8rem;color:#64748b;margin-top:2px';
        role.textContent = block.suspect_role;
      ...
      row.addEventListener('click', () => {
        ...
        row.style.borderColor = '#9d174d';
        row.style.background  = 'rgba(157,23,77,.06)';
        selectedId = block.id;
  ...
  // Knappar längst ned — fast position
  const footer = document.createElement('div');
  footer.style.cssText = 'padding:16px 20px;flex-shrink:0;border-top:1px solid #f1f5f9;display:grid;gap:8px';

  const confirmBtn = document.createElement('button');
  confirmBtn.type = 'button';
  confirmBtn.style.cssText = 'width:100%;padding:.85rem;border-radius:14px;background:#9d174d;color:#fff;border:none;font-size:1rem;font-weight:700;cursor:pointer;transition:opacity .15s';
  confirmBtn.textContent = suspects.length === 0 ? 'Avsluta utan gissning' : 'Bekräfta gissning';
  ...
  confirmBtn.addEventListener('click', () => {
    if (suspects.length > 0 && !selectedId) return;
    overlay.remove();
    const guessed = suspects.find(b => b.id === selectedId) || null;
    showMysteryReveal(guessed, suspects);
  });

  const skipBtn = document.createElement('button');
  skipBtn.type = 'button';
  skipBtn.style.cssText = 'width:100%;padding:.7rem;border-radius:14px;border:1px solid #e2e8f0;background:none;color:#64748b;font-size:.88rem;cursor:pointer';
  skipBtn.textContent = 'Hoppa över — visa svaret direkt';
  skipBtn.addEventListener('click', () => {
    overlay.remove();
    showMysteryReveal(null, suspects);
  });
...
function showMysteryReveal(guessed, suspects) {
  const culprit = suspects.find(b => b.is_culprit);
  const correct  = guessed && culprit && guessed.id === culprit.id;
  const skipped  = guessed === null;

  const existing = document.getElementById('mysteryRevealPanel');
  if (existing) existing.remove();

  const overlay = document.createElement('div');
  overlay.id = 'mysteryRevealPanel';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:200;display:flex;flex-direction:column;justify-content:flex-end';
  ...
  const body = document.createElement('div');
  body.style.cssText = 'overflow-y:auto;flex:1;padding:20px 20px 0';

  // Emoji
  const emoji = document.createElement('div');
  emoji.style.cssText = 'font-size:3rem;text-align:center;margin-bottom:8px';
  emoji.textContent = skipped ? '🔍' : correct ? '🎉' : '❌';
  body.appendChild(emoji);

  // Rubrik
  const title = document.createElement('h2');
  title.style.cssText = 'font-size:1.2rem;font-weight:700;color:#1a202c;margin:0 0 6px;text-align:center';
  title.textContent = skipped ? 'Lösningen' : correct ? 'Rätt gissning!' : 'Fel gissning';
  body.appendChild(title);

  // Ingress
  const lead = document.createElement('p');
  lead.style.cssText = 'font-size:.9rem;color:#64748b;margin:0 0 20px;text-align:center;line-height:1.55';
  if (skipped) {
    lead.textContent = culprit
      ? `Den skyldige var ${culprit.suspect_name || 'okänd'}.`
      : 'Ingen skyldig har angetts i det här uppdraget.';
  } else if (correct) {
    lead.textContent = `Du hade rätt! ${culprit?.suspect_name || ''} är skyldig.`;
  } else {
    lead.textContent = culprit
      ? `Du gissade på ${guessed?.suspect_name || 'okänd'}. Den skyldige var ${culprit.suspect_name || 'okänd'}.`
      : 'Ingen skyldig har angetts i det här uppdraget.';
  }
  body.appendChild(lead);

  // Culprit-kort om vi vet vem det är
  if (culprit) {
    const culpritCard = document.createElement('div');
    culpritCard.style.cssText = 'display:flex;align-items:center;gap:14px;padding:14px 16px;background:#fdf4ff;border:1px solid rgba(157,23,77,.2);border-radius:16px;margin-bottom:16px';
    if (culprit.suspect_image_url) {
      const img = document.createElement('img');
      img.src = culprit.suspect_image_url;
      img.style.cssText = 'width:52px;height:52px;border-radius:50%;object-fit:cover;flex-shrink:0';
      culpritCard.appendChild(img);
    }
    const culpritInfo = document.createElement('div');
    const culpritName = document.createElement('div');
    culpritName.style.cssText = 'font-size:1rem;font-weight:700;color:#9d174d';
    culpritName.textContent = culprit.suspect_name || 'Okänd';
    culpritInfo.appendChild(culpritName);
    if (culprit.suspect_role) {
      const culpritRole = document.createElement('div');
      culpritRole.style.cssText = 'font-size:.8rem;color:#64748b;margin-top:2px';
      culpritRole.textContent = culprit.suspect_role;
      culpritInfo.appendChild(culpritRole);
    }
    culpritCard.appendChild(culpritInfo);
    body.appendChild(culpritCard);
  }

  // Förklaring
  if (culprit?.culprit_explanation) {
    const expWrap = document.createElement('div');
    expWrap.style.cssText = 'background:#f8f9fa;border-left:3px solid #9d174d;border-radius:0 12px 12px 0;padding:12px 16px;margin-bottom:16px';
    const expLabel = document.createElement('div');
    expLabel.style.cssText = 'font-size:.7rem;font-weight:700;color:#9d174d;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px';
    expLabel.textContent = 'Förklaring';
    const expText = document.createElement('p');
    expText.style.cssText = 'font-size:.9rem;color:#374151;line-height:1.6;margin:0;font-style:italic';
    expText.textContent = culprit.culprit_explanation;
  ...
  finishBtn.style.cssText = 'width:100%;padding:.85rem;border-radius:14px;background:#1d4ed8;color:#fff;border:none;font-size:1rem;font-weight:700;cursor:pointer';
  finishBtn.textContent = 'Avsluta till startsidan';
  finishBtn.addEventListener('click', () => {
    overlay.remove();
    handleConfirmCompleteAssignment();
  });

  const stayBtn = document.createElement('button');
  stayBtn.type = 'button';
  stayBtn.style.cssText = 'width:100%;padding:.7rem;border-radius:14px;border:1px solid #e2e8f0;background:none;color:#64748b;font-size:.9rem;cursor:pointer';
  stayBtn.textContent = 'Stanna kvar och utforska';
...
function checkAndShowAutoComplete() {
  ...
  // Mystery hanterar avslut via gissningsflödet — inte här
  // Trail hanterar avslut via användarens aktiva val på kartan — inte här
  if (isMysteryAssignment() || isTrailAssignment()) {
    return;
  }
...
function showAutoCompleteView() {
  ...
  // Visa sammanfattning direkt utan att behöva trycka extra
  if (els.autoCompleteSummaryWrap) {
    els.autoCompleteSummaryWrap.classList.remove('hidden');
  }
  if (els.autoCompleteSummaryToggleBtn) {
    els.autoCompleteSummaryToggleBtn.classList.add('hidden');
  }

  // Sätt dynamisk knappetikett baserat på uppdragstyp
  if (els.autoCompleteFinishBtn) {
    if (isMysteryAssignment()) {
      els.autoCompleteFinishBtn.textContent = 'Avsluta mordgåtan →';
    } else if (isTrailAssignment()) {
      els.autoCompleteFinishBtn.textContent = 'Avsluta tipspromenaden →';
    } else {
      els.autoCompleteFinishBtn.textContent = 'Avsluta uppdraget →';
    }
  }
...
function openLocationGuide() {
  if (!els.locationGuideCard) return;
  els.locationGuideCard.classList.remove('hidden');
  els.locationGuideCard.removeAttribute('aria-hidden');
  if (els.locationGuideBtn) els.locationGuideBtn.setAttribute('aria-expanded', 'true');
  els.locationGuideCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function closeLocationGuide() {
  if (!els.locationGuideCard) return;
  els.locationGuideCard.classList.add('hidden');
  els.locationGuideCard.setAttribute('aria-hidden', 'true');
  if (els.locationGuideBtn) els.locationGuideBtn.setAttribute('aria-expanded', 'false');
}
...
function normalizePlace(place) {
  const contentBlocks = getContentBlocks(place).map((block) => ({
    ...block,
    // Databasen lagrar kolumnen som block_type — mappa till type för PWA-kompatibilitet
    type: block.type || block.block_type || 'text',
    // Mappa witness_image_path (från API-join) till witness_image_url (används av createWitnessNode)
    witness_image_url: block.witness_image_url ||
      (block.witness_image_path ? resolveMediaUrl(block.witness_image_path) : null),
    // Mappa suspect_image_path till suspect_image_url (används av createSuspectNode)
    suspect_image_url: block.suspect_image_url ||
      (block.suspect_image_path ? resolveMediaUrl(block.suspect_image_path) : null),
    // Mappa evidence_image_url/path till media_url (används av createEvidenceNode och renderEvidenceList)
    media_url: block.media_url ||
      block.evidence_image_url ||
      (block.evidence_image_path ? resolveMediaUrl(block.evidence_image_path) : null),
  }));
...
function getPlaceAvailability(place) {
  ...
  // Läsläge — alla platser är upplåsta efter att uppdraget är klart
  if (state.readMode) {
    return {
      visited: hasVisitedPlace(place.id),
      unlockMode: 'free',
...
function createContentBlockElement(block, place = null, availability = null) {
  ...
  if (type === 'witness') {
    const witnessNode = createWitnessNode(block);
    article.classList.add('content-block-witness');
    article.appendChild(witnessNode);
    return article;
  }

  if (type === 'evidence') {
    const evidenceNode = createEvidenceNode(block);
    article.classList.add('content-block-evidence');
    article.appendChild(evidenceNode);
    return article;
  }

  if (type === 'suspect') {
    const suspectNode = createSuspectNode(block);
    article.classList.add('content-block-witness');
    article.appendChild(suspectNode);
    return article;
  }
...
function createWitnessNode(block) {
  ...
  if (block.witness_image_url) {
    ...
    img.addEventListener('click', () => openLightbox(block.witness_image_url, img.alt));
    header.appendChild(img);
  } else {
    const avatar = document.createElement('div');
...
function createSuspectNode(block) {
  const wrapper = document.createElement('div');
  wrapper.className = 'witness-block'; // återanvänder witness-stilar
  ...
  const badge = document.createElement('span');
  badge.className = 'witness-reliability-badge';
  badge.style.background = 'rgba(157,23,77,.12)';
  ...
  if (block.suspect_motive) {
    const motiveWrap = document.createElement('div');
    motiveWrap.style.cssText = 'display:grid;gap:2px;padding:8px 14px;background:rgba(157,23,77,.04);border-left:3px solid rgba(157,23,77,.25);border-radius:0 8px 8px 0';
    const label = document.createElement('div');
    label.style.cssText = 'font-size:.72rem;font-weight:700;color:rgba(157,23,77,.7);text-transform:uppercase;letter-spacing:.05em';
    label.textContent = 'Motiv';
    const text = document.createElement('div');
    text.style.cssText = 'font-size:.88rem;color:var(--sheet-muted);font-style:italic';
    text.textContent = block.suspect_motive;
    motiveWrap.appendChild(label);
    motiveWrap.appendChild(text);
    wrapper.appendChild(motiveWrap);
  }

  if (block.suspect_alibi) {
    const alibiWrap = document.createElement('div');
    alibiWrap.style.cssText = 'display:grid;gap:2px;padding:8px 14px;background:rgba(59,130,246,.04);border-left:3px solid rgba(59,130,246,.25);border-radius:0 8px 8px 0';
    const label = document.createElement('div');
    label.style.cssText = 'font-size:.72rem;font-weight:700;color:rgba(59,130,246,.7);text-transform:uppercase;letter-spacing:.05em';
    label.textContent = 'Alibi';
    const text = document.createElement('div');
    text.style.cssText = 'font-size:.88rem;color:var(--sheet-muted);font-style:italic';
    text.textContent = block.suspect_alibi;
    alibiWrap.appendChild(label);
    alibiWrap.appendChild(text);
    wrapper.appendChild(alibiWrap);
  }
...
function isQuestionType(type) {
  return type === 'question' || type === 'quiz';
}

function isSolutionType(type) {
  return type === 'solution' || type === 'answer';
}
...
const visited = hasVisitedPlace(place.id);
const showNumbers = !!state.assignment?.show_place_numbers;
...
// Visa ordningsnummer om admin aktiverat det
if (showNumbers) {
  marker.classList.add('numbered');
  const sortedPlaces = getPlaces();
  const placeIndex = sortedPlaces.findIndex(p => String(p.id) === String(place.id));
  const num = document.createElement('span');
  num.className = 'map-marker-num';
  num.textContent = placeIndex >= 0 ? String(placeIndex + 1) : '';
