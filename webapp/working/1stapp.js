...
function bindEvents() {
  ...
  if (els.joinGroupInput) {
    els.joinGroupInput.addEventListener('keydown', (e) => {
...
async function registerServiceWorker() {
  ...
  try {
    const reg = await navigator.serviceWorker.register(`${appBasePath}/service-worker.js`, {
      scope: `${appBasePath}/`
    });

    // Kontrollera om en ny version väntar redan vid start
    if (reg.waiting) {
      showUpdateBanner(reg.waiting);
    }

    // Lyssna på nya versioner som hittas under pågående session
    ...
    console.warn('[SW] Registrering misslyckades:', err.message);
  }
}
...
function showUpdateBanner(worker) {
  ...
  btn.addEventListener('click', () => {
    ...
    // Ladda om när nya service workern tagit över
...
function getPlaceAvailability(place) {
  ...
  */
  ...
  const previousPlace = isLinearMysteryMode() ? getPreviousPlaceInSequence(place) : null;
  const sequenceBlocked = !!(previousPlace && !hasVisitedPlace(previousPlace.id) && !visited);
